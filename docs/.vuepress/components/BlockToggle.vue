  ...
  methods: {
    ...
    addExpand(hiddenHeight = 40) {
      ...
      Array.from(modes).forEach((item) => {
        if (item.parentNode.className === 'vuepress-plugin-demo-block__code') {
          return
        }
        // 添加自定义class以更改样式
        item.classList.add('custom_line-numbers-mode_block-toggle')
        // 首先获取 expand 元素
        ...
        if (!expand) {
          ...
          div.className = "expand icon-xiajiantou iconfont";
          // 箭头点击事件
          div.onclick = () => {
            ...
            if (parseInt(item.style.height) == hiddenHeight) {
              ...
              div.className = "expand icon-xiajiantou iconfont closed";
...
<style>
/* 代码块元素 */
.custom_line-numbers-mode_block-toggle {
  overflow: hidden;
  transition: height 0.3s;
  margin-top: 0.85rem;
}

.custom_line-numbers-mode_block-toggle::before {
...
.expand {
  ...
  top: 0.8em;
  right: 0.5em;
  color: rgb(23 161 161 / 80%);
  font-weight: 900;
  transition: transform 0.3s;
}

/* 代码块内容 */
div.custom_line-numbers-mode_block-toggle[class*="language-"].line-numbers-mode pre {
  margin: 30px 0 0.85rem 0;
}

/* 代码块的行数 */
div.custom_line-numbers-mode_block-toggle[class*="language-"].line-numbers-mode .line-numbers-wrapper,
.highlight-lines {
  margin-top: 30px;
}
...
/* 代码块的语言 */
div.custom_line-numbers-mode_block-toggle[class*="language-"]::before {
  position: absolute;
  z-index: 3;
  top: 0.3em;
  left: 4.7rem;
...
li div.custom_line-numbers-mode_block-toggle[class*="language-"]::before,
li .expand {
  margin-top: -4px;
}

/* 代码块行数的线条 */
div.custom_line-numbers-mode_block-toggle[class*="language-"].line-numbers-mode::after {
  margin-top: 35px;
}
...
.theme-mode-light div.custom_line-numbers-mode_block-toggle[class*="language-"]::before {
  color: #666;
}
